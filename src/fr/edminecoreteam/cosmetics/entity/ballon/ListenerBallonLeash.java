...
public class ListenerBallonLeash 
{
	public static void despawnLeash(Player p) {
        for (Entity stand : Bukkit.getServer().getWorld("hub").getEntitiesByClasses(new Class[] { Chicken.class })) {
            if (stand != null && stand.hasMetadata("leash_" + p.getName())) {
                stand.remove();
                runAction(p, "false");
            }
        }
    ...
    private static void updateLocLeash(Player p) {
        for (Entity stand : Bukkit.getServer().getWorld("hub").getEntitiesByClasses(new Class[] { Chicken.class })) {
            if (stand != null && stand.hasMetadata("leash_" + p.getName())) {
    ...
    public static void runAction(Player player, String s) {
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (Entity stand : Bukkit.getServer().getWorld("hub").getEntitiesByClasses(new Class[] { Chicken.class })) {
                    if (stand != null && stand.hasMetadata("leash_" + player.getName())) {
                    	updateLocLeash(player);
                    }
                }
                if (!player.isOnline()) {
                    cancel();
